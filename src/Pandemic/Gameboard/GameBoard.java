...
public class GameBoard implements Cloneable {
	...
	// Index of first Discard Cards in beforeEpidemicPile
	public int indexFirstDiscardedCards = 0;
	...
	public int getNeededForCure(String role) {
		if (role == "SCIENTIST") {
			return neededForCure - 1;
		} else {
	...
	public int getInfectionRate() {
		return Variables.INFECTION_RATE;
	}

	public int getOutbreakCount() {
		return Variables.OUTBREAK_MARKER;
	}

	// Increments the outbreak counter
	public void incrementOutbreak() {
		Variables.OUTBREAK_MARKER++;
	}

	// add a card to PlayerDiscardPile
	public void addPlayerCardDiscard(City toAdd) {
		playerDiscardPile.add(toAdd);
	}

	/*
	 * set the initial Infected cities you may initialize this methods with any
	 * methods you created this methods called from @SimulatePandemic class
	 **/
	...
	// This method will infect a given number of cities based on the given infection
	// rate.
		...
		while (i < infectionRate) {
			...
			for (k = 0; k < checkCities.size(); k++) {
				if (checkCities.get(k).getName().equals(cityToInfect.getName())) {
					...
					infectPile.remove(0);
					System.out.println(
							"Won't place a cube to " + cityToInfect.getName() + " because of quarantine_specialist");
					break;
				}
			}
...
//            }
			if (k == checkCities.size()) {
				System.out.println(
						"INFECTING " + cityToInfect.getName() + " with 1 " + cityToInfect.getColour() + " cubes.");
		...
		if (checkCubesRemain(infectCity.getColour())) {
			...
			for (x = 0; x < cities.size(); x++) {
				if (cities.get(x).getName().equals(infectCity.getName())) {
					// System.out.println("I have found " + infectCity.getName() + " and it is at
					// position " + x + " within the cities array");
					break;
				}
			}
			if (cities.get(x).addCube(infectCity.getColour())) {
				...
				for (e = 0; e < infectCity.getNeighbors().size(); e++) {
					...
					for (q = 0; q < cities.size(); q++) {
						if (cities.get(q).getName().equals(infectCity.getNeighbors().get(e).getName())
								&& (!checkCities1.contains(infectCity.getNeighbors().get(e).getName()))) {
							// System.out.println("I have found " + infectCity.getName() + " and it is at
							// position " + x + " within the cities array");
			...
			removeCube(infectCity.getColour());
			// System.out.println("Just removed a " + infectCity.getColour() + " from the
			// pool");
	...
	public boolean checkCubesRemain(String colour) {
		...
		if (colour == "Blue" && blueCubes > 0) {
			return toReturn = true;
		} else if (colour == "Yellow" && yellowCubes > 0) {
			return toReturn = true;
		} else if (colour == "Black" && blackCubes > 0) {
			return toReturn = true;
		} else if (colour == "Red" && redCubes > 0) {
			return toReturn = true;
		}
		return toReturn;
	}
	...
	public void infectEpidemic() {
			...
			if ((cities.get(x).addCube(toInfect.getColour()))) {
				...
				for (e = 0; e < toInfect.getNeighbors().size(); e++) {
					for (int z = 0; z < cities.size(); z++) {
						if (cities.get(z).getName().equals(toInfect.getNeighbors().get(e).getName())
								&& (!checkCities1.contains(toInfect.getNeighbors().get(e).getName()))) {
							// System.out.println("I have found " + infectCity.getName() + " and it is at
							// position " + x + " within the cities array");
		...
		// Keep index of first Discarded Cards
		indexFirstDiscardedCards = infectDiscardPile.size();
		System.out.println("Discarded: " + indexFirstDiscardedCards);

		// keep DiscardPile record after a epidemic occur
		for (x = 0; x < infectDiscardPile.size(); x++) {
			if (!infectDiscardPile.contains(infectDiscardPile.get(x))) {
				infectDiscardPileBeforeEpidemic.add(0, infectDiscardPile.get(x));
			} else {
				infectDiscardPileBeforeEpidemic.remove(infectDiscardPile.get(x));
				infectDiscardPileBeforeEpidemic.add(0, infectDiscardPile.get(x));
			}
		}
	}
	...
	public void setPlayerPieces(Piece[] playerPieces) {
	...
	public void setInfectDeck(InfectDeck infectDeck) {
		this.infectDeck = infectDeck;
	}

	public void setCitiesMap(InfectDeck citiesMap) {
		CitiesMap = citiesMap;
	}

	public void setPlayerPile(ArrayList<Object> playerPile) {
	...
	public void setInfectPile(ArrayList<City> infectPile) {
		this.infectPile = infectPile;
	}

	public void setInfectDiscardPile(ArrayList<City> infectDiscardPile) {
		this.infectDiscardPile = infectDiscardPile;
	}
	...
	public void setCheckCities(ArrayList<City> checkCities) {
		this.checkCities = checkCities;
	}

	public void setResearchCentres(ArrayList<City> researchCentres) {
		this.researchCentres = Variables.CITY_WITH_RESEARCH_STATION;
	}
	...
	public InfectDeck getInfectDeck() {
		return infectDeck;
	}

	public InfectDeck getCitiesMap() {
		return CitiesMap;
	}
	...
	public ArrayList<City> getCheckCities() {
		return checkCities;
	}

	public ArrayList<City> getResearchCentres() {
	...
	protected Object clone() throws CloneNotSupportedException {
		...
		// cloned.playerDiscardPile = (ArrayList<Object>)
		// this.getPlayerDiscardPile().clone();
		...
		for (int i = 0; i < playerdispile.size(); i++) {
			if (playerdispile.get(i) instanceof City) {
				...
				throw new CloneNotSupportedException(
						"Player discard pile contains objects that are neither City nor boolean!");
			}
		...
		// cloned.infectDiscardPile = (ArrayList<City>)
		// this.getInfectDiscardPile().clone();
		...
		// cloned.infectDiscardPileBeforeEpidemic = (ArrayList<City>)
		// this.getInfectDiscardPileBeforeEpidemic().clone();
		...
		cloned.researchCentres = clonedresearchCentres; // REMINDER, City shallow clones for neighbors, deep clones for
														// self
